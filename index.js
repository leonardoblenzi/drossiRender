...
const cors = require('cors'); // Adicionei o CORS, é boa prática para APIs
...
// ✅ FUNCIONALIDADE: Pesquisa em Descrições
try {
...
// ✅ NOVA FUNCIONALIDADE: Analytics de Palavras-chave (NOVO NOME E ROTAS)
try {
    const keywordAnalyticsRoutes = require('./routes/keywordAnalyticsRoutes'); // Novo nome
    app.use('/api/keyword-analytics', keywordAnalyticsRoutes); // Novo prefixo de rota
...
// ✅ ROTA PARA A NOVA INTERFACE HTML DE ANALYTICS DE PALAVRAS-CHAVE
try {
    app.get('/keyword-analytics', (req, res) => { // Nova rota para o HTML
    res.sendFile(path.join(__dirname, 'views/keyword-analytics.html')); // Caminho corrigido
});
    console.log('✅ Rota de interface de keyword analytics carregada');
} catch (error) {
    console.error('❌ Erro ao carregar rota de interface de keyword analytics:', error.message);
}
...
app.get('/test-basic', (req, res) => {
    res.json({
        ...
        env: {
            ...
            access_token_configured: !!process.env.MERCADOLIBRE_ACCESS_TOKEN // Verificação do token do ML
        }
    });
});
...
app.use((req, res) => {
    res.status(404).json({
        ...
        available_routes: [
            'GET /',
            'GET /test-basic',
            'GET /criar-promocao', // Se tiver uma rota HTML separada
            'GET /remover-promocao', // Se tiver uma rota HTML separada
            'GET /pesquisa-descricao',
            'GET /keyword-analytics', // Nova rota HTML
            'GET /debug/routes'
        ]
    });
});
...
const server = app.listen(PORT, () => {
    ...
    console.log(`    • http://localhost:${PORT}/criar-promocao - Interface de criação de promoções`);
    console.log(`    • http://localhost:${PORT}/remover-promocao - Interface de remoção de promoções`);
    console.log(`    • http://localhost:${PORT}/pesquisa-descricao - Pesquisa em descrições`);
    console.log(`    • http://localhost:${PORT}/keyword-analytics - Análise de Palavras-chave`); // Novo link
    console.log(`    • http://localhost:${PORT}/debug/routes - Debug de rotas`);
    console.log('🚀 ================================');
    console.log('📊 APIs disponíveis:');
    console.log(`    • http://localhost:${PORT}/api/token/ - API de Token`);
    console.log(`    • http://localhost:${PORT}/api/promocao/ - API de Promoções`);
    console.log(`    • http://localhost:${PORT}/api/pesquisa-descricao/ - API de Pesquisa em Descrições`);
    console.log(`    • http://localhost:${PORT}/api/keyword-analytics/ - API de Análise de Palavras-chave`); // Nova API
    console.log('🚀 ================================');
    console.log('⚙️ Configuração:');
    console.log(`    • Porta: ${PORT}`);
    console.log(`    • Ambiente: ${process.env.NODE_ENV || 'development'}`);
    console.log(`    • Token MercadoLibre configurado: ${process.env.MERCADOLIBRE_ACCESS_TOKEN ? '✅ SIM' : '❌ NÃO'}`);
    console.log('🚀 ================================');
